...
import { createRoute, updateRoute, updateRoutePricing } from "../lib/api/routes";
...
export function RouteManager({ initialRoutes, contracts }: { initialRoutes: Route[]; contracts: Contract[] }) {
  ...
  async function save(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (editing) {
        const updated = await updateRoute(editing.id, { origin: form.origin, destination: form.destination });
        ...
        setRoutes((current) => current.map((route) => route.id === updated.id ? { ...updated, contracts: editing.contracts } : route));
      } else {
        const created = await createRoute({ origin: form.origin, destination: form.destination, ...(form.contractId && form.rate && form.activeFrom ? { contractId: form.contractId, rate: form.rate, currency: form.currency, activeFrom: form.activeFrom, ...(form.activeTo ? { activeTo: form.activeTo } : {}) } : {}) });
  ...
  return (
    <div className="space-y-4">
      ...
      {editing ? <RouteForm form={form} setForm={setForm} contracts={contracts} saving={saving} onSubmit={save} onCancel={() => { setEditing(null); setForm(emptyForm); }} /> : null}
