...
export function RouteManager({ initialRoutes, contracts }: { initialRoutes: Route[]; contracts: Contract[] }) {
  ...
  const [sortBy, setSortBy] = useState<"origin" | "destination" | "rate" | "status">("origin");
  const [sortDirection, setSortDirection] = useState<"asc" | "desc">("asc");

  const visibleRoutes = useMemo(() => {
    const filtered = routes.filter((route) => statusFilter === "ALL" || route.status === statusFilter).filter((route) => `${route.origin} ${route.destination}`.toLowerCase().includes(query.toLowerCase()));
    return [...filtered].sort((left, right) => {
      const leftValue = sortBy === "rate" ? Number(left.contracts?.[0]?.rate ?? 0) : sortBy === "status" ? left.status : sortBy === "destination" ? left.destination.toLowerCase() : left.origin.toLowerCase();
      const rightValue = sortBy === "rate" ? Number(right.contracts?.[0]?.rate ?? 0) : sortBy === "status" ? right.status : sortBy === "destination" ? right.destination.toLowerCase() : right.origin.toLowerCase();
  ...
  async function save(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (editing) {
        ...
        if (form.contractId && form.rate && form.activeFrom) await updateRoutePricing(editing.id, { contractId: form.contractId, rate: form.rate, currency: form.currency, activeFrom: form.activeFrom, ...(form.activeTo ? { activeTo: form.activeTo } : {}) });
      } else {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-wrap items-center gap-2"><div className="flex items-center gap-2 rounded-lg border border-[#d1d9e0] bg-[#f5f7f8] px-3 py-2 text-sm text-[#6c7782] sm:w-72"><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search routes" className="w-full bg-transparent outline-none placeholder:text-[#8b97a2]" /></div><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as typeof statusFilter)} className="form-input !mt-0 !w-auto"><option value="ALL">All statuses</option><option value="ACTIVE">Active</option><option value="INACTIVE">Inactive</option></select><select value={sortBy} onChange={(event) => setSortBy(event.target.value as typeof sortBy)} className="form-input !mt-0 !w-auto"><option value="origin">Sort: origin</option><option value="destination">Sort: destination</option><option value="rate">Sort: rate</option><option value="status">Sort: status</option></select><button type="button" onClick={() => setSortDirection((direction) => direction === "asc" ? "desc" : "asc")} className="rounded-lg border border-[#d1d9e0] bg-[#f5f7f8] px-3 py-2 text-sm font-semibold text-[#526b84]">{sortDirection === "asc" ? "A-Z" : "Z-A"}</button></div>
